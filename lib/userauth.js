...
function login(options) {
  return function (req, res, next) {
    ...
    if (req.session) {
      req.session._loginReferer = formatReferer(req, options.loginPath, options.rootPath);
    }
...
module.exports = function userauth(match, options) {
  ...
  if (options.rootPath !== '/') {
    ...
    options.loginPath = path.join(rootPath, options.loginPath);
    options.loginCallbackPath = path.join(rootPath, options.loginCallbackPath);
    options.logoutPath = path.join(rootPath, options.logoutPath);
  }
  ...
  return function authMiddleware(req, res, next) {
    ...
    if (options.rootPath !== '/') {
      pathname = path.join(options.rootPath, pathname);
    }
    ...
    options.getUser(req, function (err, user) {
      ...
      if (!user) {
        ...
        var nextHandler = function () {
          ...
          redirect(res, options.loginPath + '?redirect=' + redirectURL);
        };
