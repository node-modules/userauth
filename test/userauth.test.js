...
describe('userauth.test.js', function () {
  ...
  describe('mock win32 with options.rootPath = "/hello/world"', function() {
    var app;
    before(function() {
      app = connect(
        connect.cookieParser(),
    ...
    it('should return 200 status and user info after user logined', function (done) {
      request(app)
      .get('/hello/world/win32/login/callback')
      .set('mocklogin', 1)
      .expect('Location', '/hello/world/')
      .expect(302, function (err, res) {
        should.not.exist(err);
    ...
    it('should redirect to /login when not auth user visit /user* ', function (done) {
      ...
      request(app)
      .get('/user')
      .expect('Location', '/hello/world/win32/login?redirect=%2Fuser')
      .expect('')
      .expect(302, done);
      ...
      .get('/user/foo')
      .set({ Cookie: 'cookie2=' })
      .expect('Location', '/hello/world/win32/login?redirect=%2Fuser%2Ffoo')
      .expect('')
      .expect(302, done);
      ...
      .get('/user/')
      .set({ Cookie: 'cookie2= ;foo=bar' })
      .expect('Location', '/hello/world/win32/login?redirect=%2Fuser%2F')
      .expect('')
      .expect(302, done);
      ...
      .get('/user?foo=bar')
      .set('Accept', 'application/json')
      .expect('Location', '/hello/world/win32/login?redirect=%2Fuser%3Ffoo%3Dbar')
      .expect({ error: '401 Unauthorized' })
      .expect(401, done);
    });
